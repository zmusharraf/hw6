...
public:
    ...
    HASH_INDEX_T next() 
    {
        //stop after probing m_ supports
        if(this->numProbes_ == this->m_){
            return this->npos;
        }
...
private:
    ...
    size_t numDeleted_; //deleted slots kept for loading factor
};
       ...
       :  hash_(hash), kequal_(kequal), prober_(prober)
{
    ...
    //size table to first capacity and fill with nullptr
    table_.clear();
    table_.resize(CAPACITIES[mIndex_], nullptr);

    //store resize threshold
    resizeAlpha_ = resizeAlpha;

    //start with no items and no deleted slots
    numItems_ = 0;
    numDeleted_ = 0;
...
{
    double currCapacity = (double)CAPACITIES[mIndex_];
    double currUsed = (double)(numItems_ + numDeleted_);
    double load = currUsed/currCapacity;

    if(load >= resizeAlpha_){
        resize();
    }

    //probe for existing key or first empty spot
    const KeyType& key = p.first;
    HASH_INDEX_T loc = probe(key);

    if(loc == npos){
        throw std::logic_error("No free location in hash table");
    }

    if(table_[loc] == nullptr){
        //new item
        HashItem* hi = new HashItem(p);
        table_[loc] = hi;
        numItems_++;
    } else {
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    HASH_INDEX_T oldIndex = mIndex_;

    //move to next capacity
    mIndex_++;
    //if we run out of capacities, throw logic logic_error
    HASH_INDEX_T newCap = CAPACITIES[mIndex_];

    //allocate new table and reset counts
    ...
    for(HASH_INDEX_T i = 0; i < CAPACITIES[oldIndex]; i++){
        HashItem* hi = oldTable[i];
        if(hi != nullptr){
            if(hi->deleted == false){
                const KeyType& key = hi->item.first;
                HASH_INDEX_T loc = probe(key);
                if(loc == npos){
                    delete hi;
                    throw std::logic_error("No free location during resize");
                }
                HashItem* newItem = new HashItem(hi->item);
                table_[loc] = newItem;
                numItems_++;
            }
            delete hi;
